...
def visualize_pts(pts, fig=None, bgcolor=(0, 0, 0), fgcolor=(1.0, 1.0, 1.0),
    ...
    if draw_origin:
        ...
        mlab.plot3d([0, 3], [0, 0], [0, 0], color=(0, 0, 1), tube_radius=0.1, line_width=3)
        mlab.plot3d([0, 0], [0, 3], [0, 0], color=(0, 1, 0), tube_radius=0.1, line_width=3)
        mlab.plot3d([0, 0], [0, 0], [0, 3], color=(1, 0, 0), tube_radius=0.1, line_width=3)
...
def draw_scenes(points, gt_boxes=None, ref_boxes=None, ref_scores=None, ref_labels=None, gt_labels=None, class_names=None, pred_keypoints=None):
    ...
    if gt_labels is not None and not isinstance(gt_labels, np.ndarray):
        gt_labels = gt_labels.int().cpu().numpy()
    if pred_keypoints is not None and not isinstance(pred_keypoints, np.ndarray):
        pred_keypoints = pred_keypoints.cpu().numpy()
    print('GT boxes num is: %d, Ref boxes num is: %d' % (gt_boxes.shape[0], ref_boxes.shape[0]))
    fig = visualize_pts(points, show_intensity=False, size=(2000, 2000))
    fig = draw_multi_grid_range(fig, grid_size=10, bv_range=(-60, -60, 60, 60))

    if ref_labels is not None and class_names is not None:
        class_names = np.array(class_names)
        ref_class_names = class_names[ref_labels-1]

    if gt_boxes is not None:
        # draw Center Point
        mlab.points3d(gt_boxes[:, 0], gt_boxes[:, 1], gt_boxes[:, 2], color=(255.0/255.0, 20.0/255.0, 147.0/255.0), mode='sphere', scale_factor=0.2)
        corners3d = boxes_to_corners_3d(gt_boxes)
        # fig = draw_corners3d(corners3d, fig=fig, color=(0, 0, 1), max_num=100, gt_labels=gt_labels)
        fig = draw_corners3d(corners3d, fig=fig, color=(255.0/255.0, 20.0/255.0, 147.0/255.0), gt_labels=gt_labels, max_num=100)
        # if gt_labels is None:
        #     fig = draw_corners3d(corners3d, fig=fig, color=(0, 0, 1), cls=gt_labels, max_num=100)
        # else:
    ...
    if ref_boxes is not None and len(ref_boxes) > 0:
        # draw Center Point
        mlab.points3d(ref_boxes[:, 0], ref_boxes[:, 1], ref_boxes[:, 2], color=(0, 1, 0), mode='sphere', scale_factor=0.2)
        # draw key point
        mlab.points3d(pred_keypoints[:, 0], pred_keypoints[:, 1], pred_keypoints[:, 2], color=(1, 1, 1), mode='sphere', scale_factor=0.2)
        ...
        if ref_labels is None:
            ...
            for k in range(ref_labels.min(), ref_labels.max() + 1):
                cur_color = tuple(box_colormap[k % len(box_colormap)])
                mask = (ref_labels == k)
                fig = draw_corners3d(ref_corners3d[mask], fig=fig, color=cur_color, cls=ref_scores[mask], max_num=100, gt_labels=ref_class_names[mask].tolist())
    # mlab.view(azimuth=-179, elevation=54.0, distance=104.0, roll=90.0)
    mlab.view(azimuth=0.0, elevation=0.0)
    return fig


def draw_corners3d(corners3d, fig, color=(1, 1, 1), line_width=2, cls=None, tag='', max_num=500, tube_radius=None, gt_labels=None):
    """
    ...
    for n in range(num):
        ...
        if gt_labels is not None:
            ...
            if isinstance(gt_labels, np.ndarray):
                mlab.text3d(b[6, 0], b[6, 1], b[6, 2], '%d' % gt_labels[n], scale=(0.3, 0.3, 0.3), color=color, figure=fig)
            else:
                mlab.text3d((b[6, 0]+b[4, 0])/2, (b[6, 1]+b[4, 1])/2, b[6, 2], '%s' % gt_labels[n], scale=(0.3, 0.3, 0.3), color=color, figure=fig)
